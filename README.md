...
- **Screencast back-pressure (1.1.6)** — Chrome's `Page.screencastFrameAck` is only sent once a capture has been consumed by the pipeline, so Chrome captures at the rate frames are actually used instead of flooding the server with 50+ PNGs per second over CDP. Two frames may be in flight to the client (`ACK_MAX_INFLIGHT`, default 2) so encoding overlaps with the client's decode.
